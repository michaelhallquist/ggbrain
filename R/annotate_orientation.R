...
#' @param color Text color passed to ggplot2::annotate
#' @param offset Offset (in data units) to nudge labels away from the image edge. Can be scalar or length 4 (left, right, bottom, top)
...
#' @export
annotate_orientation <- function(size = 3, color = "white", offset = 0, ...) {
  params <- list(size = size, color = color, offset = offset, extra = list(...))
...
orientation_annotations_for_slice <- function(panel_index, slices_obj, size = 3, color = "white", offset = 0, ...) {
