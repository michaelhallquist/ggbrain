...
#' @param contrasts One or more named contrast definitions. The preferred form is
#'   a single unquoted expression, \code{<name> := <expression>}. Character strings
#'   using that syntax and named character vectors are retained for compatibility
#'   and for defining multiple contrasts in one call.
#' @details
#' `define()` names a derived image that can subsequently be used by
#' `geom_brain()` or `geom_outline()`. A definition has two parts:
#'
#' - The left side of `:=` is a simple, unquoted name for the new contrast.
#' - The right side is an expression involving image names previously supplied
#'   to `images()`.
#'
#' For example, if `images()` includes images named `img1` and `img2`, use
#' `define(zdiff := img1 - img2)`, followed by `geom_brain("zdiff")`.
#' Standard arithmetic, comparison, and logical operators can be used. Bracket
#' expressions restrict values to matching voxels, for example
...
#' Category labels must be quoted strings and become factor levels. Unmatched
#' voxels remain missing unless a final `TRUE ~ "Other"` clause is supplied.
#' The legacy semicolon form is still accepted in character definitions and
#' follows the same first-match precedence.
#'
#' A single unquoted contrast can be supplied per `define()` call. To define
#' several contrasts at once, use a named character vector:
#' `define(c(diff = "img1 - img2", sum = "img1 + img2"))`. The older character
#' form, `define("diff := img1 - img2")`, also remains supported.
#'
#' Defining a contrast does not draw it. Add a layer that refers to the contrast
#' name, such as `geom_brain("diff")`. Defining it separately is useful when the
#' same contrast is reused or further restricted in multiple layers.
#'
#' @examples
...
define <- function(contrasts = NULL) {
  captured <- substitute(contrasts)
  if (is.call(captured) && identical(captured[[1L]], as.name(":="))) {
    if (length(captured) != 3L || !is.symbol(captured[[2L]])) {
      stop("Unquoted define() syntax must use a simple name on the left side of :=")
    }

    contrast_name <- as.character(captured[[2L]])
    contrast_value <- paste(deparse(captured[[3L]], width.cutoff = 500L), collapse = " ")
    contrasts <- paste(contrast_name, ":=", contrast_value)
  }
...
#'
#' @param voxel_size A positive number specifying the target voxel size in millimeters (e.g., 1 for 1mm isotropic).
#'   The slices will be resampled to this resolution before display. If the input images have larger voxels
#'   (e.g., 2.5mm), they will be upsampled to the target resolution. If the input images have smaller voxels
#'   (e.g., 0.5mm), they will be downsampled.
#' @param interpolation A character string specifying the interpolation method to use. Options are:
#'   \itemize{
#'     \item \code{"nearest"} (value 1): Nearest neighbor interpolation. Fast but can produce blocky results.
#'     \item \code{"linear"} (value 2): Linear interpolation. Smooth but may blur edges.
#'     \item \code{"cubic"} (value 5): Cubic interpolation (default). Good balance of smoothness and edge preservation.
#'     \item \code{"lanczos"} (value 6): Lanczos interpolation. High quality but slower.
#'   }
#' @return A \code{ggb} object with the target resolution settings and an action of 'set_target_resolution'
#' @details
#'   This function allows you to display brain images at a different resolution than their native voxel size.
#'   For example, if your functional MRI data have 2.5mm voxels but you want to display them at 1mm resolution
#'   for publication-quality figures, you can use \code{target_resolution(1)} to upsample.
#'
#'   Conversely, if you have high-resolution images (e.g., 0.5mm) that are slow to render or result in
#'   very large file sizes, you can downsample them using \code{target_resolution(2)} to display at 2mm.
#'
#'   The resampling is performed on each 2D slice after extraction, which is more efficient than resampling
#'   the entire 3D volume. The interpolation is applied to all images in the plot. For labeled/categorical
#'   images, nearest neighbor interpolation is always used to preserve integer values.
#'
#'   Note that upsampling does not add information to the image -- it simply provides a smoother visual
#'   representation. For the highest quality results with functional overlays on anatomical underlays,
#'   consider using a high-resolution anatomical template (e.g., 1mm) as your underlay.
#'
#' @examples
#'   t1 <- system.file("extdata", "mni_template_2009c_2mm.nii.gz", package = "ggbrain")
...
#' @export
target_resolution <- function(voxel_size, interpolation = "cubic") {
  checkmate::assert_number(voxel_size, lower = 0.1, upper = 10)
  checkmate::assert_choice(interpolation, c("nearest", "linear", "cubic", "lanczos"))

  # Map human-friendly names to imager::imresize interpolation values
  # From imager::resize documentation:
  # 1 = nearest-neighbor, 2 = moving average, 3 = linear, 4 = grid, 5 = cubic, 6 = lanczos
  interp_map <- c(
    "nearest" = 1L,
    "linear" = 3L,
    "cubic" = 5L,
  ...
  interp_value <- interp_map[[interpolation]]

  ret <- ggb$new(action = "set_target_resolution")
  ret$ggb_target_resolution <- list(
    voxel_size = voxel_size,
    interpolation = interpolation,
    interpolation_value = interp_value
  )
