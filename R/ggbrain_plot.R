...
ggbrain_plot <- R6::R6Class(
  ...
  private = list(
    ...
    pvt_text_color = NULL, # overall text color of plot
    pvt_text_color_auto = TRUE, # track whether text color should follow background
    pvt_title = NULL,
  ...
  active = list(
    ...
    bg_color = function(value) {
      if (missing(value)) {
        ...
        if (isTRUE(private$pvt_text_color_auto)) {
    ...
    text_color = function(value) {
      if (missing(value)) {
        ...
        private$pvt_text_color_auto <- TRUE
        private$pvt_text_color <- infer_text_color_from_bg(private$pvt_bg_color)
      } else {
        checkmate::assert_string(value)
        private$pvt_text_color_auto <- FALSE
  ...
  public=list(
    ...
    #' @param text_color text color of plot. If NULL, a contrasting color is chosen from the background.
    ...
    generate_plot = function(layers=NULL, slice_indices=NULL) {
      ...
      private$pvt_ggbrain_panels <- lapply(seq_len(nrow(slice_df)), function(i) {
        ...
        if (is.null(pan_i$text_color)) {
          if (isTRUE(private$pvt_text_color_auto)) {
            pan_i$text_color <- infer_text_color_from_bg(pan_i$bg_color)
          } else {
            pan_i$text_color <- private$pvt_text_color
          }
        }
          ...
          if ("orientation_params" %in% names(ann_list)) {
            ...
            if (length(has_ori)) {
              ori_ann <- dplyr::bind_rows(lapply(has_ori, function(idx) {
                ...
                if (length(params) == 1L && is.null(names(params)) && is.list(params[[1]])) {
                  params <- params[[1]] # unwrap nested list from tibble/list columns
                }
                args <- c(
                  list(
                    panel_index = i,
                    slices_obj = private$pvt_slices,
                    size = params$size %||% 3,
                    color = params$color %||% pan_i$text_color,
