...
ggbrain_layer <- R6::R6Class(
  ...
  private = list(
    ...
    add_raster = function(gg, df, value_col = NULL, n_layers, raster_args = NULL, fill_scale = NULL) {
      ...
      # For outline layers, always drop NA alpha rows generated by slice_to_outline()
      # to avoid filling the entire panel background when no alpha mapping is set.
      if (inherits(self, "ggbrain_layer_outline")) {
        df <- df[!is.na(df$value), , drop = FALSE]
        if (nrow(df) == 0L) return(gg)
      }

      # use_tile <- inherits(self, "ggbrain_layer_outline") # outlines use geom_tile to avoid sparse-raster warnings
      use_tile <- FALSE # we now have raster working for outline layers without warnings
