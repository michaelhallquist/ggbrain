...
#f <- plot(gg_obj)
#f &
#  theme(plot.background = element_rect(fill = "lightgray", color = NA), legend.background =element_rect(fill="lightgray", color=NA))

plot(gg_obj) & theme_minimal()
...
abc <- gg_obj$render() & plot_annotation(title="hello")
plot(abc)
plot(gg_obj)
```
...
plot(abspe_gg) + plot_layout(nrow = 1, ncol = 2)
...
The `repel` versions of these functions rely on [`ggrepel`](https://cran.r-project.org/web/packages/ggrepel/index.html), a useful package for pushing labels away from their specified coordinates to avoid overplotting of labels and covering of relevant data points.
