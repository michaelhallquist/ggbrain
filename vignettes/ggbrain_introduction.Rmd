...
#ggbrain(bg_color = "gray80", text_color = "black", title="Rostral PPC")
...
```{r}
x <- gg_obj$render() + plot_annotation(theme = theme(plot.background = element_rect(fill = "blue", color = NA)))
```
...
gg_obj <- gg_base +
  geom_brain(definition = "underlay") +
  geom_brain(definition = "overlay[abs(overlay) > 2.5 & underlay > 10]",
             fill_scale = scale_fill_bisided(), show_legend = TRUE
  )
plot(gg_obj)
```
...
gg_obj <- ggbrain() +
  ...
  geom_brain(definition="underlay") +
  geom_outline(definition = "onset[abs(onset) > 3]", outline_size=1, outline="cyan")
plot(gg_obj)
```
...
Another point is that by keeping your plots as `ggbrain` object -- that is not adding `render()` -- you can continue to add brain-related calls like `geom_brain()` or `annotate_coordinates()` to them.
...
```{r, warning=FALSE, message=FALSE}
ggsave(filename="my_brain_plot.pdf", gg_obj$render(), width=15, height=10)
```

```{r, warning=FALSE, message=FALSE}
pdf("my_brain_plot_2.pdf", width=10, height=10)
plot(gg_obj)
dev.off()
```
...
Now that you understand the basics of `ggbrain` plots, I encourage you to read the [ggbrain aesthetics](ggbrain_aesthetics.html) vignette.
